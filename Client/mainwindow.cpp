    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(functionalityForm, SIGNAL(user_disc()), this, SLOT(on_disconnectButton_clicked()));

    for (int i = 0; i < 3; i++)
        permissions[i] = 0;

    setWindowTitle("RAT-Client application");
    ...
    connect(this, SIGNAL(send_user_info(QTcpSocket*, const char*, QString, QString)), functionalityForm, SLOT(get_user_info(QTcpSocket*, const char*, QString, QString)));
    connect(this, SIGNAL(send_screenshot_for_sharing(QPixmap*)), functionalityForm, SLOT(get_screenshot(QPixmap*)));
    connect(this, SIGNAL(enable_file_system()), functionalityForm, SLOT(get_enable_file_system()));
    connect(this, SIGNAL(send_file_sys(QString)), functionalityForm, SLOT(get_file_sys(QString)));
    connect(this, SIGNAL(hide_all()), functionalityForm, SLOT(get_hide_all()));
...
void MainWindow::socketReady()
{
    if (socket->waitForConnected(500))
    {
            ...
            bufferData[12] == 's')    // Если было получено сообщение об успехе
        {
            ...
            emit send_user_info(socket, permissions, userName, sysInfo);
            ...
            bufferData[12] == 's')
        {
            ...
            emit send_file_sys(filesys);
            functionalityForm->setEnabled(true);
            emit enable_file_system();
            return;
        }
        ...
        if (bufferData.contains("operation"))
        {
            ...
            case 'v':
                ...
                break;
            case 't':
                operation += "Текущее время:\n";
                break;
            case 'd':
                operation += "Текущая дата:\n";
                break;
            case 'z':
                operation += "Текущий часовой пояс:\n";
                break;
            }

            for (int i = 12; i < bufferData.size(); i++)
        ...
        if (bufferData.contains("Fail"))    // Ошибка верификации
        {
        ...
        if (bufferData.contains("Error"))
        {
            this->setEnabled(false);
            functionalityForm->setEnabled(false);
            QMessageBox::critical(this, "Error", "Неверный путь!");
            this->setEnabled(true);
            functionalityForm->setEnabled(true);
            emit enable_file_system();
            return;
